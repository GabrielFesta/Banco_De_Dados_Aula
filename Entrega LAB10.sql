...
create table veiculo (idVeiculo int not null unique primary key, placa
char(10) not null, modelo varchar(45) not null, cor varchar(45) not null,
idcliente integer,
...
(3, 1048, '2022-04-12', '2022-04-19', '13:34:00', '05:55:00');
 ...
 -- 1
 select c.nomeCliente, v.placa FROM (cliente as c) join (veiculo as v) ON c.idCliente = v.idCliente ORDER BY c.nomeCliente;
 
 -- 2
 
  select c.nomeCliente, v.placa FROM (cliente as c) left join (veiculo as v) ON c.idCliente = v.idCliente ORDER BY c.nomeCliente;
  
  -- 3
   select c.nomeCliente, v.placa FROM (cliente as c) right join (veiculo as v) ON c.idCliente = v.idCliente ORDER BY c.nomeCliente;
   
   -- 4 
   select c.cpf, c.nomeCliente FROM (cliente as c) join (veiculo as v) ON c.idCliente = v.idCliente WHERE v.placa = "ZU78JU49";

   -- 5
   Select v.modelo, v.placa, v.cor FROM (veiculo as v) join (estaciona as e) ON e.idVeiculo = v.idVeiculo WHERE e.idPatio = 1;
   
   -- 6
   Select v.modelo, v.placa, v.cor FROM (veiculo as v) join (estaciona as e) ON e.idVeiculo = v.idVeiculo WHERE e.idPatio = 2;
   
   -- 7
   Select v.modelo, v.placa, v.cor FROM (veiculo as v) join (estaciona as e) ON e.idVeiculo = v.idVeiculo; 
   
   -- 8 
      Select v.modelo, v.placa, v.cor FROM (veiculo as v) join (estaciona as e) ON e.idVeiculo = v.idVeiculo WHERE v.cor = "prata";
      
      -- 9
      Select v.modelo, v.placa, v.cor FROM (veiculo as v) join (estaciona as e) ON e.idVeiculo = v.idVeiculo WHERE v.cor = "vermelho";
      
      -- 10
      Select v.modelo, v.placa, e.dataEntrada FROM (veiculo as v) join (estaciona as e);
      
      -- 11 
      Select v.modelo, v.placa, e.dataEntrada FROM (veiculo as v) join (estaciona as e) ON v.idVeiculo = e.idveiculo WHERE e.dataEntrada like "2021%";
